...
//SD_ReadSDisk/SD_WriteSDisk函数专用buf,当这两个函数的数据缓存区地址不是4字节对齐的时候,
...
SD_Error SD_Init(void)
{
  ...
  NVIC_InitStructure.NVIC_IRQChannel = SDIO_IRQn;
	...
	NVIC_Init(&NVIC_InitStructure);	//根据指定的参数初始化VIC寄存器、
 	...
 	if(errorstatus==SD_OK)errorstatus=SD_SelectDeselect((u32)(SDCardInfo.Rlative_ADD<<16));//选中SD卡   
	...
	{  		    
		...
		{
			clkdiv=SDIO_TRANSFER_CLK+2;	//V1.1/V2.0卡，设置最高48/4=12Mhz
		}else clkdiv=SDIO_TRANSFER_CLK;	//SDHC等其他卡，设置最高48/2=24Mhz
		...
		else if(SD_WORK_MODE==1)
		errorstatus=SD_SetDeviceMode(SD_DMA_MODE);//设置为查询模式
 	}
	return errorstatus;		 
}
//SDIO时钟初始化设置
...
SD_Error SD_PowerON(void)
{
	...
	u32 SDType=SD_STD_CAPACITY;
	 ...
	 /*初始化时的时钟不能大于400KHz*/ 
  SDIO_InitStructure.SDIO_ClockDiv = SDIO_INIT_CLK;	/* HCLK = 72MHz, SDIOCLK = 72MHz, SDIO_CK = HCLK/(178 + 2) = 400 KHz */
...
SD_Error SD_GetCardInfo(SD_CardInfo *cardinfo)
{
	...
	cardinfo->Rlative_ADD=(u16)RCA;							//卡RCA值
