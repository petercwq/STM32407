...
static void _InitContext(GUI_CONTEXT* pContext) {
  ...
  #endif
  pContext->pAFont       = GUI_DEFAULT_FONT;  //设置的默认字体
  pContext->pClipRect_HL = &GUI_Context.ClipRect;
  pContext->PenSize      = 1;
  /* Variables in WM module */
  #if GUI_WINSUPPORT  //支持视窗管理器
    pContext->hAWin    = WM_GetDesktopWindow();  //即将被重绘的窗体句柄
  #endif
  /* Variables in GUI_AA module */
  #if GUI_SUPPORT_AA   //抗锯齿
...
int GUI_Init(void) {
  ...
  GUI_X_Init();
  /* Init context */
  _InitContext(&GUI_Context); //初始化一个上下文结构体
  GUITASK_INIT();  //任务初始化
  r = LCD_Init();   //液晶初始化
  #if GUI_WINSUPPORT
    WM_Init();    //视窗初始化
