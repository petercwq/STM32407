...
*   that no check on the parameters needs to be performed.
*/
unsigned int LCD_L0_GetPixelIndex(int x, int y) //读点函数(取得某点的像素值，并返回)
{
...
*       LCD_L0_XorPixel
*/
void LCD_L0_XorPixel(int x, int y)  //反转像素函数
{
...
void LCD_L0_FillRect(int x0, int y0, int x1, int y1) {
  ...
  //原来函数的内容是先调用画线函数,而画线函数又是调用的画点函数因为每填充一个点
	//就要设定一次点的位置,所以这是液晶刷屏慢的主要原因
	LCD_Fill_onecolor(x0,y0,x1,y1,LCD_COLORINDEX);   //改为自己的画矩形函数
}
